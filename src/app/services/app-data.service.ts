...
export class AppDataService {
  ...
  createCountry(vm: Country): Observable<any> {
    //return of({}).delay(2000).flatMap(x=>Observable.throw('Unable to create country'));
    let id = 0;
  ...
  deleteCountry(id: number): Observable<any> {
    //return of({}).delay(2000).flatMap(x=>Observable.throw('Delete error.'));
    return of({}).pipe(
      delay(2000),
      tap((e) =>
        this.countries.splice(
          this.countries.findIndex((c) => c.id == id),
          1
        )
      )
    );
